export default function Item({props, removeItem, removeBool, selectItem}){
    let {id, name, quantity, category, size, emoji} = props;

        return(
        <div onClick={() => selectItem(id)} id={id} className="box-content relative size-32 border-3 border-dotted border-[#DC143C] p-3 m-3 rounded-sm bg-[#FFFFFF]/65">
            {removeBool == true ? <button id={id} type="button" onClick={removeItem} className="absolute top-0 left-0 rounded-[50] bg-[#000000] text-[#FFFF] w-5 h-5 z-1">-</button>:<div></div>}
