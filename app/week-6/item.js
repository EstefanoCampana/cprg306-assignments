...
export default function Item({props}){
    ...
    let named = props.name.split(",")[0]
    let emoji = props.name.charAt(name.length - 1)
    let size = "bulk";
    if(props.name.includes(",")){
        size = props.name.split(",")[1]
    }
    return(
        <div className="box-content size-32 border-3 border-dotted border-[#DC143C] p-3 m-3 rounded-sm bg-[#FFFFFF]/65">
            ...
            <div className="relative size-15 mb-1">
                <div className="absolute size-32 pt-10 text-center text-sm text-[#000000]/75">{named}</div>
            </div>
